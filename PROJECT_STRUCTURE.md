...
| `docs/` | De publieke webapp (GitHub Pages): cursusoverzicht, modulenavigatie, reader (incl. visuals, luistermodus, vragenpaneel). Alleen deze map is vanaf de gepubliceerde site bereikbaar. |
| `prompts/` | Prompts: het leerprofiel voor de vertaalstap, en een apart prompt voor de interactieve Q&A-rol. |
| `deploy/` | Voorbereide, niet-geactiveerde infrastructuur (nu: een Cloudflare Worker voor de Q&A-backend onderweg — zie PIPELINE.md). |
...
data/term_familiarity.json   (cursusoverstijgende term-ledger, zie PIPELINE.md)
```
...
| `translate_module.py` | **De echte vertaalstap**: vertaalt één module (alle lessen samen, niet los) naar de Daan-leerstijl via OpenAI, met visuals en progressive terminology immersion → `course/<cursus>/translated/module-<n>.md`. Kost echt geld — zie PIPELINE.md. |
...
| `qa_server.py` | Lokale (127.0.0.1-only) Q&A-backend voor het interactieve vragenpaneel — hergebruikt je bestaande `OPENAI_API_KEY`, geen nieuw account nodig. Zie PIPELINE.md. |
...
| `js/visuals.js` | Rendert Mermaid-diagrammen uit de module-markdown (zie PIPELINE.md). |
| `js/audio.js` | Luistermodus: speech-preparation-laag + afspeel-engine (Web Speech API). |
| `js/qa.js` | Interactief vragenpaneel: FAB-knop, snelle acties, vrij tekstveld, context-tracking. |
...
| `css/style.css` | Het volledige ontwerp: donker, groen accent, mobile-first met laptop-breakpoint, incl. audiobalk/vragenpaneel/visuals. |
